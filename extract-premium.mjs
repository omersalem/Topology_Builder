// Simple extraction of original icons - no background removal
...
async function extractOriginal() {
  try {
    ...
    // Crop tightly to just the icon, exclude label
    const topPadding = 8;
    const sidePadding = 8;
    ...
    for (const icon of icons) {
      ...
      console.log(`Extracted premium: ${icon.name}.png (${width}x${height})`);
    }
    
    console.log('Done! Premium icons extracted.');
  } catch (error) {
