// Simple extraction - just crop without processing
...
async function extractSimple() {
  try {
    ...
    // Just crop tightly around the icon, exclude the label
    const topPadding = 15;
    const sidePadding = 15;
    const labelHeight = 55;
    
    for (const icon of icons) {
      const left = icon.col * iconWidth + sidePadding;
      const top = icon.row * rowHeight + topPadding;
      const width = iconWidth - (sidePadding * 2);
      const height = rowHeight - labelHeight - topPadding;
      ...
      console.log(`Extracted: ${icon.name}.png (${width}x${height})`);
    }
    
    console.log('Done! Icons extracted without quality loss.');
  } catch (error) {
